...
## Input parameters:
# Physical parameters:
mass_arr = [1.0, 10] # array of masses of particles [m1,m2]
...
r_arr = 0.0:0.5:20.0
v_arr = v_ho.(r_arr)
v_interpol = linear_interpolation(r_arr,v_arr,extrapolation_bc=Line())
...
phys_params_i = make_phys_params2B(;mur,vint_arr=[v_int],dim=2)
...
e2_exact = ([2*i + abs(0) for i=0:simax-1] .+ 1) .*omega # m=0
...
if vscale_bool == 1
    ...
    println("vscale = $(round(vscale,digits=8)) should be approximately 0.25")
end
