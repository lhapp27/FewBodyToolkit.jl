...
masses=[1.0,10.0] # finite masses of the two particles
...
# We can also use complex-ranged basis functions, which are useful for more oscillatory bound states, i.e. highly excited states. Note that `complex_ranged=true` effectively employs twice the number of basis functions, hence for a fair comparison we choose `nmaxC = nmax/2 = 7`. Keep in mind that the optimal parameters for the complex-ranged basis functions are usually different. Hence, we optimize them separately.
nmaxC = 7
r1C = 0.5; rnmaxC = 10.0;
