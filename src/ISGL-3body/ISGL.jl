...
export ISGL_solve, ISGL_matrices, make_phys_params3B3D, make_num_params3B3D
...
Arguments and keywords are the same as for [`ISGL_solve`](@ref). Intended for problems which are more naturally posed in terms of the matrices than of the energies, in particular the inverse problem, see [`inverse_solve`](@ref).
