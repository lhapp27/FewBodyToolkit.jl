...
function size_estimate(phys_params,num_params,observ_params,complex_ranged_r::Bool,complex_ranged_R::Bool)
    
    # input interpretation:
    (;species,interactions,parity) = phys_params #13diff
    (;lmax,Lmax,gem_params,complex_scaling_angle,complex_range_freq,kmax_interpol,lmin,Lmin) = num_params
    ...
    central_indices, gauss_indices, gaussopt_arr, contact1D_indices, contact1Dopt_arr, nint_arr, nintmax = index_interaction_types(interactions)
...
function index_interaction_types(interactions)
...
# function to determine list of (l,L)-tuples to consider, based on parity,species
function lLcoupl(parity,lmax,Lmax,cvals,species,lmin,Lmin)
    ...
    for c in cvals # we only need  (l,L) list for the relevant jacobi sets
        for l = lmin:lmax
            for L = Lmin:Lmax
                ...
                if species[mod(c+1,3)+1] == species[(mod(c,3)+1)] == :b  # if Jacobi-Set c contains relative coordinate between two identical bosons -> only even l
                    (mod(l,2) == 0) && push!(lL_arr[c],(l,L))
                elseif species[(mod(c+1,3)+1)] == species[(mod(c,3)+1)] == :f# if Jacobi-Set c contains relative coordinate between two identical bosons -> only odd l
