...
gaussopt=[[1,v0,mu_g]]
phys_params = (;hbar=1.0,mur,vint_arr=[v_poschl],lmax=0,dim=1)
# numerical parameters:
gem_params = (;nmax=10,r1=0.1,rnmax=10.0) # gem_params
num_params = (;gem_params,omega_cr=0.5,theta_csm=0.0,threshold=10^-10)
...
println("\ncsm_bool = 0:")
energies_arr = GEM2B.GEM_solve(phys_params,num_params;wf_bool=0,cr_bool=0,csm_bool=0)
comparison(energies_arr,lambda)


#1b. Is the output for csm_bool = 0 the same as before? with cr
println("\ncsm_bool = 0, cr_bool = 1:")
energies_arr = GEM2B.GEM_solve(phys_params,num_params;wf_bool=0,cr_bool=1,csm_bool=0)
comparison(energies_arr,lambda)

#2. Does  csm_bool = 1, theta_csm = 0.0 recover the old results?
println("\ncsm_bool = 1, theta_csm = 0.0:")
energies_arr = GEM2B.GEM_solve(phys_params,num_params;wf_bool=0,cr_bool=0,csm_bool=1)
comparison(energies_arr,lambda)

#3. Any bugs with theta_csm != 0.0?
num_params = (;gem_params,omega_cr=0.5,theta_csm=5.0,threshold=10^-10)
println("\ncsm_bool = 1, theta_csm = $(num_params.theta_csm):")
energies_arr = GEM2B.GEM_solve(phys_params,num_params;wf_bool=0,cr_bool=0,csm_bool=1)
comparison(energies_arr,lambda)

#4. check functionality of gaussopt
phys_params = (;hbar=1.0,mur,vint_arr=[v_gauss],lmax=0,dim=1)
...
energies_arr = GEM2B.GEM_solve(phys_params,num_params;wf_bool=0,cr_bool=0,csm_bool=1, gaussopt=gaussopt)
comparison(energies_arr,lambda;gauss_bool=1)

#4. Benchmark to check if code became slower
bench_bool = 0
if bench_bool == 1
    println("\n Benchmarking:") # value of nmax? probably 10
    ...
    @btime GEM2B.GEM_solve(phys_params,num_params;wf_bool=0,cr_bool=0,csm_bool=0,gaussopt=gaussopt);
    print("\n gaussopt = 0, csm_bool = 1: ")
    @btime GEM2B.GEM_solve(phys_params,num_params;wf_bool=0,cr_bool=0,csm_bool=1);
    print("\n gaussopt = 1, csm_bool = 1: ")
    @btime GEM2B.GEM_solve(phys_params,num_params;wf_bool=0,cr_bool=0,csm_bool=1,gaussopt=gaussopt);
end 
